...
// Global custom universe — unlike makeCustomUniverse (Thai default, plain → .BK),
// plain symbols stay as-is (US), and crypto (-USD) / foreign listings (.HK etc.)
// keep their provider suffixes. Used by the unified Market Scanner's custom box.
function makeGlobalCustomUniverse(customSymbols = []) {
  return (Array.isArray(customSymbols) ? customSymbols : [])
    .map((raw) => String(raw || "").trim().toUpperCase().replace(/[^A-Z0-9.^-]/g, ""))
    .filter(Boolean)
    .map((raw) => {
      if (raw.endsWith(".BK")) return makeStock(raw.slice(0, -3), raw.slice(0, -3), "SET", "CUSTOM");
      if (raw.endsWith("-USD")) {
        return { displaySymbol: raw.slice(0, -4), providerSymbol: raw, name: raw.slice(0, -4), market: "CRYPTO", assetType: "crypto", currency: "USD", universe: "CUSTOM" };
      }
      return { displaySymbol: raw, providerSymbol: raw, name: raw, market: "GLOBAL", assetType: "US_STOCK", currency: "USD", universe: "CUSTOM" };
    });
}

// Foreign stocks with Thai DRs listed on SET (scan the UNDERLYING listing — DR
// .BK symbols are thin). displaySymbol = underlying; drNote = the Thai DR series.
function makeDrStock(providerSymbol, name, drNote) {
  const display = providerSymbol.replace(/\.HK$/, "");
  return { displaySymbol: display, providerSymbol, name, market: "DR", assetType: "US_STOCK", currency: "USD", universe: "DR", drNote };
}
const DR_UNIVERSE = [
  ["NVDA", "NVIDIA Corp", "NVDA80"],
  ["TSLA", "Tesla Inc", "TSLA80"],
  ["AAPL", "Apple Inc", "AAPL80"],
  ["MSFT", "Microsoft Corp", "MSFT80"],
  ["AMZN", "Amazon.com Inc", "AMZN80"],
  ["GOOG", "Alphabet Inc", "GOOG80"],
  ["META", "Meta Platforms", "META80"],
  ["NFLX", "Netflix Inc", "NFLX80"],
  ["AMD", "Advanced Micro Devices", "AMD80"],
  ["TSM", "Taiwan Semiconductor (ADR)", "TSMC80"],
  ["ASML", "ASML Holding (ADR)", "ASML80"],
  ["BABA", "Alibaba Group (ADR)", "BABA80"],
  ["JD", "JD.com (ADR)", "JD80"],
  ["PDD", "PDD Holdings (ADR)", "PDD80"],
  ["NIO", "NIO Inc (ADR)", "NIO80"],
  ["NTES", "NetEase (ADR)", "NTES80"],
  ["0700.HK", "Tencent Holdings", "TENCENT80"],
  ["1810.HK", "Xiaomi Corp", "XIAOMI80"],
  ["1211.HK", "BYD Company", "BYDCOM80"],
  ["3690.HK", "Meituan", "MEITUAN80"]
].map(([sym, name, dr]) => makeDrStock(sym, name, dr));

// Top-10 crypto by market cap (ex-stablecoins) — Yahoo -USD listings.
function makeCryptoStock(base, name) {
  return { displaySymbol: base, providerSymbol: `${base}-USD`, name, market: "CRYPTO", assetType: "crypto", currency: "USD", universe: "CRYPTO10" };
}
const CRYPTO10_UNIVERSE = [
  ["BTC", "Bitcoin"], ["ETH", "Ethereum"], ["BNB", "BNB"], ["SOL", "Solana"], ["XRP", "XRP"],
  ["DOGE", "Dogecoin"], ["ADA", "Cardano"], ["TRX", "TRON"], ["LINK", "Chainlink"], ["AVAX", "Avalanche"]
].map(([sym, name]) => makeCryptoStock(sym, name));
...
function getThaiStockUniverse(universe = "SET100", options = {}) {
  ...
  if (key === "DR") return DR_UNIVERSE;
  if (key === "CRYPTO10" || key === "CRYPTO") return CRYPTO10_UNIVERSE;
  if (key === "CUSTOM") return makeCustomUniverse(options.customSymbols);
  if (key === "CUSTOM_GLOBAL") return makeGlobalCustomUniverse(options.customSymbols);
