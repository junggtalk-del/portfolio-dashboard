...
const { yahooChartJson } = require("../http");
...
async function fetchYahooBars(symbol, startDate, endDate) {
  ...
  const payload = await yahooChartJson(
    `/v8/finance/chart/${encodeURIComponent(providerSymbol)}?period1=${period1}&period2=${period2}` +
      `&interval=1d&events=history&includeAdjustedClose=true`,
    { timeoutMs: 8000, userAgent: "portfolio-dashboard-backtest/1.0" }
  );
  ...
  for (let index = 0; index < timestamps.length; index += 1) {
    ...
    rows.push({
      date: toIsoDate(timestamps[index] + gmtoffset),
      open: Number(open),
      high: Number(high),
      low: Number(low),
      close: Number(close),
